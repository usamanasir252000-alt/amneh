  ...
  return (
    <div className="flex items-center gap-1.5">
      <div className="flex gap-0.5">
        {[1, 2, 3, 4, 5].map((i) => (
          <svg key={i} className={`h-3.5 w-3.5 ${i <= count ? "text-gray-700" : "text-gray-300"}`} fill="currentColor" viewBox="0 0 20 20">
    ...
    return (
      <main className="min-h-screen bg-[#faf5f6]">
        ...
        <div className="flex flex-col items-center justify-center min-h-screen gap-4">
          ...
          <Link href="/" className="text-xs uppercase tracking-widest border border-gray-900 px-6 py-2.5 hover:bg-gray-900 hover:text-white transition">
            Back to Home
          </Link>
        </div>
      </main>
      ...
      <div className="max-w-screen-xl mx-auto px-6 lg:px-10 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
          ...
          <div className="flex flex-col">
            ...
            {/* Tagline / Description */}
            {product.tagline && (
              <p className="text-sm font-medium text-gray-700 mb-2">{product.tagline}</p>
            )}
            {product.description && (
              <p className="text-sm leading-7 text-gray-500 mb-6">{product.description}</p>
            )}
            {!product.tagline && !product.description && (
              <p className="text-sm leading-7 text-gray-500 mb-6">
                A luxurious formula crafted for radiant, healthy-looking skin. Part of the amneh. signature collection.
              </p>
            )}
            ...
            <div className="space-y-3">
              {[
                { icon: "M5 13l4 4L19 7", label: "Free shipping on orders over $50" },
                { icon: "M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15", label: "Free returns within 30 days" },
